...
let
  organellePathsList =
    cellsFrom:
    cell: organelle: inputs.nixpkgs.lib.reverseList (builtins.attrValues (organellePaths cellsFrom cell organelle));
  prefixWithCellsFrom =
    path:
...
in
  {
    ...
    OnePathImport =
      ...
      in
        if
          builtins.isAttrs imported && !inputs.nixpkgs.lib.isDerivation imported
        then
          abort
            ''
